...
# 이미지 입력 함수 cv2.imread()를 통해 로컬 경로의 이미지를 읽어옴
image = cv2.imread("C:\study\image\cat1.jpg", cv2.IMREAD_ANYCOLOR)  # cv2.imread(fileName, flags)
                                                                    # 파일경로(fileName)의 이미지 파일을 플래그(flags) 설정에 따라 불러옴
                                                                    # 파일경로(fileName)은 상대경로, 절대경로 두가지 방법으로 불러올수 있음
                                                                    # 플래그(flags)는 이미지를 초기에 불러올때 초기 상태를 의미함

# 이미지 표시 함수 cv2.imshow()와 키 입력 대기 함수 cv2.waitKey()로 윈도우창에 이미지를 띄움                                                                 
cv2.imshow("Cat", image)
cv2.waitKey()   # 키 대기 함수를 입력하지 않을 경우 윈도우 창이 유지되지 않고 프로그램 종료
cv2.destroyAllWindows() # 키 입력후 윈도우 창 제거 함수 cv2.destroyAllWindows()를 이용하여 모든 윈도우 창을 닫음